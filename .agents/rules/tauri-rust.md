...
Applies to native code under `src-tauri/`.
...
Do not place large business rules, raw SQL, or synchronization orchestration directly inside commands.
...
Normal application functionality must continue working offline.
...
Persist user changes locally first.
...
Entities that may be created offline must use client-generated globally unique IDs.

Prefer UUID or UUIDv7 where appropriate.

Do not rely on remote auto-increment IDs for offline-created entities.
...
Before finishing relevant Rust changes, run:
